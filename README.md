...
- **Windows 2000 shell icons.** A token cannot repaint an inline `currentColor` glyph, so the chrome sheet hides a glyph's own children and paints a period bitmap in its place: yellow folders (closed and open), the tree's + / − expander boxes, a document on a Session row that has no status dot, the settings gear, a padlock on the permission chip, and a black triangle on the composer and hero dropdowns. The hooks are the DOM's own: `aria-expanded` on the workspace row, `data-slot` on the chips, and the empty leading slot a Session row leaves behind.
...
| Composer and hero dropdowns (`[data-slot="conversation.input.permission"]`, `…input.model`, `…hero.agentPreset` — their `svg[width="14"]`) | A combo box's black triangle | Rule: replace the hairline chevron with a 6×4 black triangle, so a win2k combo reads as one. |
