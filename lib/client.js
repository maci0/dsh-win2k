 ...
 *
 * While the flag is on the applier re-forces the theme on every
 * `theme/change`, so a click on Light/Dark/System bounces back and the cube
 * says so. That is the honest reading of a persistent theme this plugin owns:
 * the durable `ui-theme` schema holds built-in ids only, so the alternative —
 * letting a built-in click win silently — would drop the choice on the next
 * reload.
 *
 ...
 * is scoped under `body[data-dsw-win2k]`, an attribute this half toggles from
 * the theme snapshot, so the sheet stays inert under light/dark/system. It is
 * appended after ui-theme's sheets (this package injects
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.dw-group{display:flex;flex-direction:column;gap:8px;padding:16px 0;'
      ...
      '.dw-cube{box-sizing:border-box;flex:0 1 180px;display:flex;flex-direction:column;'
        + 'align-items:center;justify-content:center;gap:4px;padding:20px 32px;'
        + 'border:0.5px solid var(--dsw-alias-border-l4);border-radius:20px;background:transparent;'
      ...
      '.dw-cube-selected{background:var(--dsw-alias-bg-module-platform);'
    ...
    const en = {
      ...
      override: 'The Appearance cubes above are overridden while this is on; switch it off to go back.',
    ...
    const zh = {
      ...
      override: '开启期间上方「外观」的选项会被覆盖；关闭后即可恢复。',
    ...
    /**
     * Mount every surface: the theme registration, the applier, the chrome
     * scope, and the cube row.
    ...
    function apply(ctx) {
      ...
      ctx.effect(
        () => theme.register({ id: THEME_ID, label: t('label'), colorScheme: 'light', tokens: TOKENS }),
      ...
      const wanted = () => {
        ...
        return selectedOf(scope.getSnapshot()) === true
      }

      /**
       * Keep the theme service on the flag's side. The durable theme preference
       * cannot hold `win2k`, so the re-force is what survives a reload; a click
       * on a built-in cube therefore bounces back while this is on.
       */
      const sync = () => {
        if (typeof document !== 'undefined') {
          document.body.toggleAttribute(ACTIVE_ATTRIBUTE, theme.getTheme().active.id === THEME_ID)
        }
      ...
      const toggle = (next) => {
        if (scope.getSnapshot().writable) {
          // The write comes back through the scope subscription, so the theme
